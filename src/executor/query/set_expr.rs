...
impl Glue {
	...
	pub async fn from_body(
		...
		match body {
			...
			SetExpr::Insert(_) => Err(Error::Query(QueryError::QueryNotSupported)),
			SetExpr::Query(query) => self.query(*query).await,
		}
	}
	#[async_recursion(?Send)]
	pub async fn from_body_no_mut(
		&self,
		body: SetExpr,
		order_by: Vec<OrderByExpr>,
	) -> Result<LabelsAndRows> {
		match body {
			...
			SetExpr::SetOperation {
				...
				let (left_labels, left) = self.from_body_no_mut(*left, vec![]).await?;
				let (right_labels, right) = self.from_body_no_mut(*right, vec![]).await?;
				if left_labels.len() != right_labels.len() {
					return Err(Error::Query(QueryError::OperationColumnsMisaligned));
				}
				let mut rows = match op {
					Union => [left, right].concat(),
					Except => left
			...
			SetExpr::Query(query) => self.no_cte_query(*query).await,
			SetExpr::Insert(_) => Err(Error::Query(QueryError::QueryNotSupported)),
		}
	}
}
